...
// Düzenlenecek kuponun bilgilerini, bu firmaya ait olup olmadığını kontrol ederek çek.
...
if (!$kupon) {
    die("Böyle bir kupon bulunamadı veya düzenleme yetkiniz yok.");
}
?>
<!DOCTYPE html>
<html lang="tr">
...
<div class="container">
    ...
    <form action="kupon_duzenle_isle.php" method="POST">
        <input type="hidden" name="coupon_id" value="<?php echo $kupon['id']; ?>">

        <label for="code">Kupon Kodu:</label>
        <input type="text" name="code" value="<?php echo htmlspecialchars($kupon['code']); ?>" required>

        <label for="discount_rate">İndirim Oranı (%):</label>
        <input type="number" name="discount_rate" value="<?php echo htmlspecialchars($kupon['discount_rate']); ?>" required>

        <label for="usage_limit">Kullanım Limiti:</label>
        <input type="number" name="usage_limit" value="<?php echo htmlspecialchars($kupon['usage_limit']); ?>" required>

        <label for="expiration_date">Son Kullanma Tarihi:</label>
        <input type="date" name="expiration_date" value="<?php echo htmlspecialchars($kupon['expiration_date']); ?>" required>

        <button type="submit">Değişiklikleri Kaydet</button>
    </form>
    <a href="kupon_yonetimi.php">Kupon Yönetimine Geri Dön</a>
</div>
</body>
</html>